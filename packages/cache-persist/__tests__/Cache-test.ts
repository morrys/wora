...
const INITIAL_STATE = { restore: true }
...
describe('storage', () => {
    ...
    it('cache restored', async () => {
        ...
        it('storage', async () => {
            ...
            await storage.getAllKeys().then(keys => expect(keys.length).toBe(2))
...
describe('Cache, others', () => {
    ...
    it('cache debounce', async () => {
        ...
        await cache.restore();
        Array.from(Array(10000).keys()).forEach(index => { cache.set(`prova${index}`, index) });
        Array.from(Array(3500).keys()).forEach(index => { cache.remove(`prova${index}`) });
        await sleep(600);
        //jest.runOnlyPendingTimers();
        expect(storage.multiRemove).toHaveBeenCalledTimes(1);
        expect(storage.multiSet).toHaveBeenCalledTimes(1);
        Array.from(Array(20).keys()).forEach(index => { cache.set(`prova${index}`, index) });
        Array.from(Array(20).keys()).forEach(index => { cache.remove(`prova${index}`) });
        await sleep(600);
        expect(storage.multiRemove).toHaveBeenCalledTimes(2);
        Array.from(Array(20).keys()).forEach(index => { cache.set(`prova${index}`, index) });
        Array.from(Array(20).keys()).forEach(index => { cache.delete(`prova${index}`) });
        await sleep(600);
