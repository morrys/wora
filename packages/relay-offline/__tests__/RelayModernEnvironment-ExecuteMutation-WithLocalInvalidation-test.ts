...
describe('executeMutation() with local invalidation', () => {
    ...
    beforeEach(async () => {
        ...
        CommentQuery = graphql`
            query RelayModernEnvironmentExecuteMutationWithLocalInvalidationTestCommentQuery($id: ID!) {
                node(id: $id) {
                    id
        ...
        CreateCommentMutation = graphql`
            mutation RelayModernEnvironmentExecuteMutationWithLocalInvalidationTestCreateCommentMutation($input: CommentCreateInput!) {
    ...
    describe('when record invalidated inside updater after server payload', () => {
        ...
        it('correctly invalidates the record when query is written after invalidation', async () => {
            // Execute mutation
            const selector = createReaderSelector(CommentFragment, commentID, {}, queryOperation.request);
            const snapshot = environment.lookup(selector);
            const callback = jest.fn();
